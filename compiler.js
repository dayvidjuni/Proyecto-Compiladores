...
class Parser {
    ...
    parseEventList() {
        const events = [];
        while ([TokenType.SHOW, TokenType.DIALOGUE, TokenType.CHOICE, TokenType.SET, TokenType.IF, TokenType.GOTO].includes(this.currentToken.type)) {
            ...
            if (this.mainFlowIndex < this.mainFlow.length) {
                ...
                if (mainStmt.nodeType === 'SceneCallNode') {
                    if (this.scenes.has(mainStmt.sceneId)) {
                        this.currentScene = this.scenes.get(mainStmt.sceneId);
                        this.currentEventIndex = -1; // Start from the beginning of the new scene
                         console.log(`Entering scene: ${this.currentScene.id}`);
                        // Immediately try to advance into the first event of the new scene
                        return this.advance();
                    } else {
    ...
    executeEvent(event) {
        switch (event.nodeType) {
            ...
            case 'ShowCharacterNode':
                ...
                // In a real web version, you'd update the UI here (e.g., show image)
                return null; // Instant action, continue processing
